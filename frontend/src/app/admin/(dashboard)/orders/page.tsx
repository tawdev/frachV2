...
export default function AdminOrders() {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/orders/${id}/delete`, {
        method: 'POST',
      });
      if (response.ok) {
    ...
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 transition-transform hover:scale-105 border-l-4 border-l-green-400">
          <p className="text-sm text-text-muted mb-1 font-medium">Livrées</p>
